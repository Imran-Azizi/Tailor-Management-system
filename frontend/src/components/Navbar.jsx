...
import {
  LuSearch, LuBell, LuSun, LuMoon, LuLanguages, LuCheck,
  LuMenu, LuTriangleAlert, LuUser, LuLogOut, LuShieldCheck,
  LuChevronDown,
} from 'react-icons/lu';
...
const ROLE_COLORS = {
  ADMIN: '#2563EB', DOKAN: '#7C3AED', DOKHT: '#DB2777', QICHIKAR: '#D97706',
};

// Emergency + worker-status notifications (for Admin)
function SystemNotifDropdown({ onClose }) {
  ...
  return (
    <div className="dd-menu" style={{ width: 340, right: 0 }}>
      <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontWeight: 600, fontSize: 14 }}>{t('navbar.notifications')}</span>
        {emergency.length > 0 && (
          <button onClick={() => { navigate('/notifications'); onClose(); }} style={{ fontSize: 12, color: 'var(--primary)', fontWeight: 500, cursor: 'pointer', background: 'none', border: 'none' }}>
            {t('navbar.viewAll')}
          </button>
        )}
      </div>
      ...
      {!hasAny ? (
        <div style={{ padding: '24px 16px', textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>{t('navbar.allCaughtUp')}</div>
      ) : (
        <div style={{ maxHeight: 400, overflowY: 'auto' }}>
          {/* Worker status updates */}
          {workerNotifs.length > 0 && (
            <>
              <div style={{ padding: '8px 14px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface2)' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Work Updates</span>
                <button onClick={() => readAllWorkerMut.mutate()} disabled={readAllWorkerMut.isPending}
                  style={{ fontSize: 11, color: 'var(--text3)', cursor: 'pointer', background: 'none', border: 'none', fontWeight: 500 }}>
                  Mark all read
                </button>
              </div>
              {workerNotifs.slice(0, 6).map((n) => (
                <div key={n.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
                  <LuBell size={13} style={{ color: '#2563EB', flexShrink: 0, marginTop: 2 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 12, lineHeight: 1.4, color: 'var(--text1)' }}>{n.message}</p>
                    <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{new Date(n.createdAt).toLocaleString()}</p>
                  </div>
                  <button onClick={() => readWorkerMut.mutate(n.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', display: 'flex' }}>
                    <LuCheck size={13} />
                  </button>
                </div>
              ))}
          ...
          {emergency.length > 0 && (
            <>
              <div style={{ padding: '8px 14px 6px', background: 'var(--surface2)' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#EF4444', textTransform: 'uppercase', letterSpacing: '.06em' }}>Emergency Alerts</span>
              </div>
              {emergency.slice(0, 6).map((n) => (
                <div key={n.id} className="dd-item" style={{ alignItems: 'flex-start', gap: 10, padding: '11px 14px' }}>
                  <LuTriangleAlert size={14} style={{ color: '#EF4444', flexShrink: 0, marginTop: 2 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--text1)' }}>{n.message}</p>
                    <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{new Date(n.createdAt).toLocaleString()}</p>
                  </div>
                  <button onClick={() => readEmergencyMut.mutate(n.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', display: 'flex' }}>
                    <LuCheck size={13} />
                  </button>
                </div>
              ))}
...
function UserNotifDropdown({ onClose }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  const navigate = useNavigate();
  const { data: notifs = [] } = useQuery({
    queryKey: ['my-notifs-nav'],
    queryFn: () => api.get('/users/me/notifications?unread=true').then(r => r.data),
  });
      ...
      <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {notifs.length > 0 && (
            <>
              <button
                onClick={() => { navigate('/my-tasks'); onClose(); }}
                style={{ fontSize: 12, color: 'var(--primary)', fontWeight: 500, cursor: 'pointer', background: 'none', border: 'none' }}
              >
                {t('navbar.viewAll')}
              </button>
              <button
      ...
      {notifs.length === 0 ? (
        ...
        <div style={{ maxHeight: 340, overflowY: 'auto' }}>
          {notifs.slice(0, 8).map((n) => (
            <div key={n.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '11px 14px', borderBottom: '1px solid var(--border)', cursor: 'default' }}>
              <LuBell size={13} style={{ color: '#2563EB', flexShrink: 0, marginTop: 3 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--text1)' }}>{n.message}</p>
                <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3 }}>{new Date(n.createdAt).toLocaleString()}</p>
              </div>
              <button
    ...
    <div className="dd-menu">
      ...
      {langs.map((lang) => (
        <div
          ...
          onClick={() => { i18n.changeLanguage(lang.code); localStorage.setItem('lang', lang.code); onClose(); }}
        >
          <span style={{ fontSize: 12, fontWeight: 800 }}>{lang.flag}</span>
          <span>{lang.label}</span>
          {current === lang.code && <LuCheck size={13} style={{ marginLeft: 'auto' }} />}
        </div>
      ))}
    </div>
  );
...
function UserDropdown({ onClose }) {
  ...
  return (
    <div className="dd-menu" style={{ width: 210, right: 0 }}>
      ...
      {isAdmin && (
        <div className="dd-item" onClick={() => { navigate('/users'); onClose(); }}>
          <LuShieldCheck size={14} />
          <span>{t('users.title')}</span>
        </div>
      )}
      <div className="dd-item" onClick={handleLogout} style={{ color: '#EF4444' }}>
        <LuLogOut size={14} />
...
export default function Navbar({ onHamburger, pageTitle }) {
  ...
  return (
    <header className="navbar no-print">
      <button className="nav-btn" onClick={onHamburger} style={{ display: 'none' }} id="ham-btn">
        <LuMenu size={20} />
      </button>
      ...
      <div className="dd-wrap" ref={langRef}>
        <button className="nav-btn" onClick={() => { setLangOpen((o) => !o); setNotifOpen(false); setUserOpen(false); }}>
          <LuLanguages size={17} />
          <span className="nbl">{(i18n.resolvedLanguage || 'en').slice(0, 2).toUpperCase()}</span>
        </button>
        {langOpen && <LangDropdown onClose={() => setLangOpen(false)} />}
      </div>

      <button className="nav-btn" onClick={toggle} title={dark ? t('common.lightMode') : t('common.darkMode')}>
        {dark ? <LuSun size={17} /> : <LuMoon size={17} />}
      </button>

      <div className="dd-wrap" ref={notifRef}>
        <button className="nav-btn" style={{ position: 'relative' }} onClick={() => { setNotifOpen((o) => !o); setLangOpen(false); setUserOpen(false); }}>
          <LuBell size={18} />
          {unreadCount > 0 && (
            <span className="notif-dot" style={{ minWidth: 16, height: 16, borderRadius: 99, background: '#EF4444', color: '#fff', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'absolute', top: -4, right: -4, padding: '0 4px' }}>
        ...
        {notifOpen && (
          isWorker
            ? <UserNotifDropdown onClose={() => setNotifOpen(false)} />
            : <SystemNotifDropdown onClose={() => setNotifOpen(false)} />
        )}
      </div>
