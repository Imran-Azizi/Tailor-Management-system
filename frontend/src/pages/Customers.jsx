...
export default function Customers({ openCreate = false }) {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setEditing(null);
      toast.success(
        editing ? t("customersPage.updated") : t("customersPage.created"),
      );
    },
    onError: (error) =>
      toast.error(getApiErrorMessage(error, t("customersPage.saveFailed"))),
  });
  ...
  const delMut = useMutation({
    ...
    onError: (error) =>
      toast.error(getApiErrorMessage(error, t("customersPage.deleteFailed"))),
  });
  ...
  return (
    <div className="page">
        ...
        action={
          <div className="search-bar">
            ...
            <button className="btn btn-gold" onClick={openCreateModal}>
              <LuPlus size={15} /> {t("customersPage.addCustomer")}
            </button>
          </div>
        }
      />
                ...
                <tbody>
                  {!data?.data?.length ? (
                    <tr>
                      <td colSpan={6}>
                        <EmptyState
                          message={t("customersPage.noCustomersYet")}
                          Icon={LuUsers}
                        />
                      </td>
                    </tr>
      ...
      <Modal
        ...
        title={
          editing ? t("customersPage.modalEdit") : t("customersPage.modalNew")
        }
      >
        <form
        ...
        >
          <Field
            label={t("customersPage.firstName")}
          ...
          <Field
            label={t("customersPage.phoneNumber")}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            ...
            >
              {saveMut.isPending
                ? t("customersPage.saving")
                : t("customersPage.save")}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDeleteModal
        ...
        title={t("customersPage.deleteTitle", {
          defaultValue: t("common.delete"),
        })}
        message={t("customersPage.deleteConfirmDetail", {
          name: deleteCustomerTarget?.name || "-",
          defaultValue:
            "Delete this customer and related records permanently? This action cannot be undone.",
        })}
        itemName={
