...
import { PageHeader, Spinner, EmptyState, ConfirmDeleteModal, NotificationText } from '../components/ui/index.jsx';
...
export default function Notifications() {
  ...
  const readMut = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, t('notificationsPage.markReadFailed'))),
  });
  ...
  const readAllMut = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, t('notificationsPage.markAllReadFailed'))),
  });
  ...
  const delMut = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, t('notificationsPage.deleteFailed'))),
  });
  ...
  return (
    <div className="page">
      ...
      {isLoading ? <Spinner /> : !notifs.length ? (
        <div className="card" style={{ padding: 40 }}><EmptyState message={t('notificationsPage.empty')} Icon={LuBell} /></div>
      ) : (
          ...
          {notifs.map((n) => (
            <div key={n.id} className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'flex-start', gap: 14, borderLeft: !n.isRead ? '3px solid var(--primary)' : '3px solid transparent' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <NotificationText language={language} style={{ fontSize: 13, lineHeight: 1.5 }}>
                  {formatSystemNotificationMessage(n, t, language)}
                </NotificationText>
                <div style={{ display: 'flex', gap: 14, marginTop: 5, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, color: 'var(--text3)' }}>{t('notificationsPage.created')}: {formatDateTimeLocale(n.createdAt, language)}</span>
                  <span style={{ fontSize: 11, color: 'var(--text3)' }}>{t('notificationsPage.next')}: {formatDateTimeLocale(n.nextAlert, language)}</span>
                  {n.expiresAt && <span style={{ fontSize: 11, color: '#DC2626' }}>{t('notificationsPage.expires')}: {formatDateLocale(n.expiresAt, language)}</span>}
                </div>
              </div>
      ...
      <ConfirmDeleteModal
        ...
        title={t('notificationsPage.deleteTitle', { defaultValue: t('common.delete') })}
        message={t('notificationsPage.deleteConfirm', {
          defaultValue: 'Delete this notification permanently? This action cannot be undone.',
        })}
