...
export async function createUser(req, res, next) {
  try {
    ...
    if (rawPassword.length < 6) {
      return res.status(400).json({ error: 'Phone number must be at least 6 characters to use as default password.' });
    }
...
export async function readNotification(req, res, next) {
  try {
    ...
    await prisma.userNotification.update({ where: { id: req.params.id }, data: { isRead: true } });
